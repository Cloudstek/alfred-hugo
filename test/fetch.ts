...
test.beforeEach((t) => {
    ...
    t.context.urlHash = crypto.createHash("md5").update(t.context.url).digest("hex");
...
test.serial("fetch uncached", async (t) => {
    ...
    t.false(h.cache.has(t.context.urlHash));
...
test.serial("fetch cached", async (t) => {
    ...
    t.deepEqual(h.cache.get(t.context.urlHash), { message: "hello" });
    ...
    t.deepEqual(h.cache.get(t.context.urlHash), { message: "world" });
